...
const existing = [
  { name: 'Teen Wolf', symbol: 'TWOLF', address: '0x1111111111111111111111111111111111111111' },
...
describe('anti-vamp identity protection', () => {
  it('normalizes names and symbols for exact on-chain-style uniqueness', () => {
    expect(normalizeTokenName('  Teen   Wolf!! ')).toBe('teen wolf');
    expect(normalizeTokenSymbol(' tw0lf ')).toBe('TWOLF');
  });

  it('blocks exact duplicate normalized names and symbols', () => {
    const result = assessTokenIdentity({ name: 'teen wolf', symbol: 'TWOLF' }, existing);
  ...
  it('blocks homoglyph name collisions because they normalize to the same protected name', () => {
    const result = assessTokenIdentity({ name: 'Teen W0lf', symbol: 'TEENW' }, existing);
    expect(result.status).toBe('blocked');
    expect(result.reasons).toContain('DUPLICATE_NAME');
    expect(result.matches[0]?.symbol).toBe('TWOLF');
  });

  it('warns on fuzzy near-miss names that are not exact normalized duplicates', () => {
    const result = assessTokenIdentity({ name: 'Teen Wolfs', symbol: 'TEENW' }, existing);
